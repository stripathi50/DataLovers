...
```{r}
#7. What factors affect poverty in countries? 
loadPkg("ggplot2")
loadPkg("gridExtra")
loadPkg("ggcorrplot")
...
Income_pop <- ggplot(Adata9) +
    geom_point(aes(x = Income , y = TotalPop, size = 2 , main = "Population vs Income"), alpha = 0.7 , col = "red")
abline(lm(Adata9$TotalPop ~ Adata9$Income), col = "blue", lwd = 3)
text(paste("Correlation:", round(cor(Adata9$Income, Adata9$TotalPop), 2), x= 50000 , y = 60000))
Unemployment_Pop <- ggplot(Adata9) +
    geom_point(aes(x = Unemployment , y = TotalPop, size = 2, main = "Population vs Unemployement"), alpha = 0.7, col = "red")
abline(lm(Adata9$Unemployment ~ Adata9$Income), col = "blue", lwd = 3)
text(paste("Correlation:", round(cor(Adata9$Income, Adata9$TotalPop), 2),x = 10, y = 60000))

grid.arrange(Income_pop, Unemployment_Pop, ncol=2, nrow=1)

#Correlation employed vs Income
library(ggcorrplot)
Check5 <- data.frame(Adata9$Employed,Adata9$Income)
data(Check5)
corr <- round(cor(Check5), 1)
ggcorrplot(corr, hc.order = TRUE, outline.col = "white")
#Correlation unemployement vs Income
Check6<- data.frame(Adata9$Unemployment,Adata9$Income)
data(Check6)
corr <- round(cor(Check6), 1)
ggcorrplot(corr, hc.order = TRUE, outline.col = "white")
#Correlation population vs unemployement
Check7<- data.frame(Adata9$TotalPop,Adata9$Unemployment)
data(Check7)
corr <- round(cor(Check7), 1)
ggcorrplot(corr, hc.order = TRUE, outline.col = "white")
#Correlation population vs Income
Check8<- data.frame(Adata9$TotalPop,Adata9$Income)
data(Check8)
corr <- round(cor(Check8), 1)
ggcorrplot(corr, hc.order = TRUE, outline.col = "white")
```
...
P-value for 'Professional' is 0.04 which is less than 0.05, we reject the null and state that 'Professional' is a affecting factor on 'Poverty'.
...
P-value for 'Service' is 0.01 which is smaller than 0.05, we reject the null and state that 'Service' is an affecting factor for 'Poverty'.
...
P-value for 'Production' is 0.8 which is greater than 0.05, we can not reject $H_0$, and can not conclude that 'Production' is an affecting factors for 'Poverty'.

```{r WorkAtHome test,results='hold'}
set.seed(123)
...
```{r PublicWork test,results='hold'}
set.seed(123)
...
```{r SelfEmployed test,results='hold'}
set.seed(123)
...
```{r UnEmployment test,results='hold'}
set.seed(123)
...
P-value for 'Unemployment' is $1*10^{-6}$, so we have sufficient evidence to reject null and conclude that it is an effecting factor for 'Poverty'.
...
**In this case, 'State' is an effecting factor for 'Poverty'.

```{r State ANOVA test, results='hold'}
anostate<-aov(Poverty~State,data = Adata9)
summary(anostate)
```

As we can see in the results of ANOVA test, our p-value is extremely small, so we have sufficient evidence to reject the null and conclude that there is difference in 'Poverty' for different states. Finally, we will perform a Post-hoc test to see the pairs of states that have different 'Poverty' as below.

```{r State Post-hoc test,results='hold'}
tukeyAoV <- TukeyHSD(anostate)
tukeyAoV
```

As a conclusion for all the tests we performed above, the effecting factors for 'Poverty' are 'Income', 'Professional', 'Service', 'WorkAtHome', 'Unemployment', and 'State'.
